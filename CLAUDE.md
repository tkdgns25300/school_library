...
[브라우저]
   │
   ▼
[Vercel Edge CDN]  ← 6개 list 페이지는 prerender본 직접 서빙 (10~50ms)
   │ ↓ MISS
   ▼
...
| 페이지 | 모드 | 이유 |
|---|---|---|
| `/`, `/students`, `/books`, `/teachers`, `/loans`, `/loans/history` | `'use cache'` (CDN 캐시) | 데이터 변경 빈도 낮음, 모든 교사가 같은 뷰 |
...
| 태그 | 무효 트리거 | 의존 쿼리 |
|---|---|---|
| `students` | 학생 CRUD/CSV | `getStudentsWithStats`, `getClassStats`, `getLoansForMonitoring`, `getLoanHistory` |
| `books` | 책 CRUD/CSV | `getBooksWithStatus`, `getStudentsWithStats`, `getLoansForMonitoring`, `getLoanHistory` |
...
**라우트 (6페이지 + 운영)**: `/` 대여 데스크 홈 · `/operation/[section]` 반별 대여 데스크 · `/loans` 대여 현황 · `/loans/history` 대여 내역 · `/students` · `/teachers` · `/books`
